...
mlst_code_remove=Set()
mlst_id_remove=Set()
sub_mlst_id_remove=Set()

line = open('./obsolete/mlst_code_remove.txt')
for x in line.readlines():
	mlst_code_remove.add(x.rstrip('\n'))
if len(mlst_code_remove) != 0:
	if '' not in mlst_code_remove:
		for mlst_code in mlst_code_remove:
			cur.execute("""SELECT mlst_id FROM inf_modifier_lists WHERE mlst_code = %s""",(mlst_code,))
			for mlst_id in cur:
				mlst_id_remove.add('%d'%int(mlst_id[0]))
for sub_mlst_remove in mlst_id_remove:
#	cur.execute("""DELETE FROM inf_modifier_lookup WHERE mdlu_type = 'm' AND mdlu_mlst_id = %s""",(sub_mlst_remove,))
	cur.execute("""SELECT mdlu_sub_mlst_id FROM inf_modifier_lookup WHERE mdlu_type = 'm' AND mdlu_mlst_id = %s""",(sub_mlst_remove,))
	for sub_mlst_id in cur:
		sub_mlst_id_remove.add('%d'%int(sub_mlst_id[0]))
...
while 1:
	...
	for x in all_occured_mlu:
		...
		for y in cur:
			if y[0] != 0 and '%d'%int(y[0]) not in sub_mlst_id_remove:
				temp.add('%d'%int(y[0]))
...
#print meta_mlu
sec_meta_mlu=Set()
for fir_meta_mlu in meta_mlu:
	cur.execute("""SELECT mdlu_sub_mlst_id FROM inf_modifier_lookup WHERE mdlu_mlst_id = %s """,(fir_meta_mlu,))
	for sec in cur:
		if sec[0] != 0:
			sec_meta_mlu.add('%d'%int(sec[0]))
#print sec_meta_mlu
#######################################################################################
...
for x in cur:
	if '%d'%int(x[0]) not in meta_mlu:
		cur.execute("""DELETE FROM inf_modifier_lists WHERE mlst_id = %s""",(x,))
#######################################################################################
cur.execute("""SELECT item_id,item_modi1_mlst_id,item_modi2_mlst_id,item_modi3_mlst_id FROM inf_items""")
for x in cur:
	temp=Set()
	tmp=Set()
	temp.add('%d'%int(x[1]))
...
#	print tmp
	for y in temp:
		if y != 0:
			cur.execute("""SELECT mdlu_sub_mlst_id FROM inf_modifier_lookup WHERE mdlu_mlst_id = %s AND mdlu_type = 'm'""",(y,))
			for z in cur:
				tmp.add('%d'%int(z[0]))
	length=len(tmp)
	if length >9:
	...
	for k in tmp:
		if cnt <= max:
#			print 'UPDATE inf_items SET item_modi%s_mlst_id = %s where item_id = %s'%(cnt,k,x[0])
			cur.execute("""UPDATE inf_items SET item_modi%s_mlst_id = %s where item_id = %s""",(cnt,k,x[0]))
			cnt+=1
cur.execute("""DELETE FROM inf_modifier_lookup WHERE mdlu_type = 'm'""")
###################eliminate all nested modifer sub list in record#####################
